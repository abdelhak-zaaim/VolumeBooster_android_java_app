...
class CircularProgressBar(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        var lcolor = attributes.getColor(R.styleable.CircularProgressBar_cpb_progressbar_color_start, 0);
        if (lcolor != 0) progressBarColorStart = lcolor

        lcolor=attributes.getColor(R.styleable.CircularProgressBar_cpb_progressbar_color_start, 0);
        if (lcolor != 0) progressBarColorStart = lcolor
        lcolor=attributes.getColor(R.styleable.CircularProgressBar_cpb_progressbar_color_end, 0);
        if (lcolor != 0) progressBarColorEnd = lcolor
        ...
        lcolor=attributes.getColor(R.styleable.CircularProgressBar_cpb_background_progressbar_color_start, 0);
        if (lcolor != 0) backgroundProgressBarColorStart = lcolor;
        lcolor=attributes.getColor(R.styleable.CircularProgressBar_cpb_background_progressbar_color_end, 0);
        if (lcolor != 0) backgroundProgressBarColorEnd = lcolor
    ...
    fun setProgressWithAnimation(progress: Float,
        ...
        progressAnimator?.addUpdateListener {
            if (indeterminateMode) progressIndeterminateMode = (it.animatedValue as Float) else this.progress = (it.animatedValue as Float)
            if (indeterminateMode) {
                val updateAngle = (it.animatedValue as Float) * 360 / 100
                startAngleIndeterminateMode = DEFAULT_START_ANGLE +
        ...
        progressAnimator?.addUpdateListener { animation ->
            {
                if (indeterminateMode) progressIndeterminateMode = (animation.animatedValue as Float) else this.progress = (animation.animatedValue as Float)
                if (indeterminateMode) {
                    val updateAngle = (animation.animatedValue as Float) * 360 / 100
                    startAngleIndeterminateMode = DEFAULT_START_ANGLE +
                            if (progressDirectionIndeterminateMode.isToRight()) updateAngle else -updateAngle
                }
            }
